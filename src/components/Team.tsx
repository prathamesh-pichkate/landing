...
const TeamMember = ({ name, role, bio, image, imageClassName, expertise = [], github, linkedin }: TeamMemberProps) => {
          ...
          <CardContent className="p-8 text-center h-full flex flex-col justify-between">
            <div>
              <div className="w-24 h-24 rounded-full overflow-hidden mx-auto mb-6 shadow-lg">
                <img src={image} alt={name} className={`w-full h-full object-cover transition-transform duration-300 hover:scale-105 ${imageClassName || ''}`} />
              </div>
          ...
          <CardContent className="p-8 h-full flex flex-col justify-between">
            <div className="text-center">
              ...
              <div className="space-y-3 text-sm text-muted-foreground">
                <div className="bg-card/50 p-3 rounded-lg"><strong>Role:</strong> {role}</div>
                {expertise.length > 0 && (
                  <div className="bg-card/50 p-3 rounded-lg">
                    <strong>Expertise:</strong> {expertise.join(", ")}
                  </div>
                )}
                <div className="bg-card/50 p-3 rounded-lg"><strong>Contributions:</strong> {bio}</div>
              </div>
            </div>
